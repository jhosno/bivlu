...
<ul class="breadcrumb">
    ...
    <li><a href="">{{$perspective=='admin'? 'Solicitudes Virtuales' : 'Mis Solicitudes de Préstamos'}}</a></li> 
</ul>
...
@endif
  @foreach($cuentas[$value->id] as $key2 => $value2)
                  ...
                  <tr> 
                      ...
                      <td>{{ $value2 }}</td>
                      <td> 
                          <a title="Visualizar" href="{{ url('virtuales/'.$value->id.'/'. 
...
function cancelar(id)
{
  $.confirm({
    ...
    content : 'Prodeca si ya no necesita el ejemplar solicitado.',
    buttons: {
      confirm:
      { 
        text:'Confirmar',
