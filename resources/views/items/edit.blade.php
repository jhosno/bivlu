<div class="modal-header">mi editar {{ isset($libro) ? $libro['titulo'] : '' }}</div>
  ...
  <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
    ...
    <div class="col-md-3">
     {{ $libro->url!=null? 'TESIS' : 'LIBRO' }}
        ...
        <div class="col-md-3">
           <input class="form-control"  title="Ingrese la Cota del libro." placeholder="Ej. REF HD7273R 73 T2" name="correlativo" type="text"  value="{{ isset($item) ? $item['correlativo'] : '' }}" id = "correlative"required value="onchange.uniqid()"> 
       </div>
 </div>
 <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
